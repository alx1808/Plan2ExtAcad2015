...
namespace Plan2Ext
{
	public class Flaeche
	{
		...
		{
			return AktFlaecheCommon(doc, rbName, flAttrib, periAttrib, fgLayer, afLayer, selectAll, layerSchalt, automated, null);
		}
		...
		internal static List<AktFlaecheErrorType> AktFlaeche(
			...
			string fgLayer,
			string afLayer,
			IEnumerable<AreaEngine.FgRbStructure> fgRbStructures
		)
		{
			return AktFlaecheCommon(doc, rbName, flAttrib, periAttrib, fgLayer, afLayer, false, false, false, fgRbStructures);
		}


		private static List<AktFlaecheErrorType> AktFlaecheCommon(_AcAp.Document doc, string rbName, string flAttrib, string periAttrib, string fgLayer,
			string afLayer, bool selectAll, bool layerSchalt, bool automated, IEnumerable<AreaEngine.FgRbStructure> fgRbStructures)
		{
			log.Debug("--------------------------");
			...
			try
			{
				...
				Dictionary<_AcDb.ObjectId, AreaEngine.FgRbStructure> fgRbStructs;
				if (fgRbStructures == null)
				{
					var areaEngine = new AreaEngine();
					if (!areaEngine.SelectFgAndRb(_FlaechenGrenzen, _Raumbloecke, _FgLayer, _RaumblockName, selectAll))
						return errorList;
					fgRbStructs = AreaEngine.GetFgRbStructs(_RaumblockName, _FgLayer, _AfLayer, doc.Database);
					var orphans = AreaEngine.OrphanRaumblocks;
				}
				else
				{
					// fgrb-Structures already exist
					var rbs = new HashSet<_AcDb.ObjectId>();
					fgRbStructs = new Dictionary<_AcDb.ObjectId, AreaEngine.FgRbStructure>();
					foreach (var f in fgRbStructures)
					{
						fgRbStructs[f.FlaechenGrenze] = f;
						foreach (var rbid in f.Raumbloecke)
						{
							rbs.Add(rbid);
						}
					...
					_FlaechenGrenzen.AddRange(fgRbStructs.Keys);
					_Raumbloecke.Clear();
					_Raumbloecke.AddRange(rbs);
				...
				using (_AcDb.Transaction transaction = tm.StartTransaction())
				{
					...
					for (int i = 0; i < _FlaechenGrenzen.Count; i++)
					{
						...
						if (!GetRegionArea(elFG, ssAF, out dM2, out dPeri, tm, out errorSymbolSet))
						{
							...
							if (!errorList.Contains(AktFlaecheErrorType.InvalidGeometry))
								errorList.Add(AktFlaecheErrorType.InvalidGeometry);
							continue;
						}
						...
						if (rbAnz < 1)
						{
							...
							if (!errorList.Contains(AktFlaecheErrorType.NoRaumBlock))
								errorList.Add(AktFlaecheErrorType.NoRaumBlock);
							fehlerKeinRb++;
						}
						else if (rbAnz > 1)
						{
							...
							//FehlerLineOrHatchPoly(elFG, _InvalidNrRb, 255, 0, 0, tm, Globs.GetMiddlePoint(elFG));
							if (!errorList.Contains(AktFlaecheErrorType.MoreThanOneRaumBlock))
								errorList.Add(AktFlaecheErrorType.MoreThanOneRaumBlock);
							fehlerMehrRb++;
						}
						else
						{
							...
							foreach (var attRef in attRefs)
							{
								if (string.Compare(attRef.Tag, _FlAttrib, StringComparison.OrdinalIgnoreCase) == 0)
								{
									...
									{
										log.DebugFormat(
											"Unterschied in Raumblock {0}: Wert in Attribut = {1}, tatsächlicher Wert = {2}.",
											blockRef.Handle.ToString(), attRef.TextString, sAtt);
										Differs = true;
										if (_Modify)
										{
											attRef.UpgradeOpen();
							...
							if (Differs && (!_Modify))
							{
								...
								if (!errorList.Contains(AktFlaecheErrorType.WrongM2))
									errorList.Add(AktFlaecheErrorType.WrongM2);
								fehlerWertFalsch++;
							}
						}
					}
					...
					if (_Raumbloecke.Count > 0)
					{
						...
						{
							_AcIntCom.AcadBlockReference rbBlock =
								(_AcIntCom.AcadBlockReference)Globs.ObjectIdToAcadEntity(_Raumbloecke[i], tm);
							insPoints.Add(rbBlock.InsertionPoint);
						}

						if (!errorList.Contains(AktFlaecheErrorType.RaumblocksWithoutFlaechengrenze))
							errorList.Add(AktFlaecheErrorType.RaumblocksWithoutFlaechengrenze);
					...
					if (fehlerKeinRb > 0 || fehlerMehrRb > 0 || fehlerWertFalsch > 0 || _Raumbloecke.Count > 0)
					{
						string msg = string.Format(CultureInfo.CurrentCulture,
							"Räume ohne Raumblock: {0}\nRäume mit mehr als einem Raumblock: {1}\nRäume mit falschem Wert in Raumblock: {2}\nRaumblöcke ohne entsprechende Flächengrenzen: {3}",
							fehlerKeinRb, fehlerMehrRb, fehlerWertFalsch, _Raumbloecke.Count);
						if (attsNotFound > 0)
						{
							msg += string.Format(CultureInfo.CurrentCulture, "\nBlöcke ohne Flächenattribut: {0}!",
								attsNotFound.ToString());
						}
		...
		internal static void AktFlaecheOld(_AcAp.Document doc, string rbName, string flAttrib, string periAttrib, string fgLayer, string afLayer)
		{
			...
			try
			{
				...
				if (!areaEngine.SelectFgAndRb(_FlaechenGrenzen, _Raumbloecke, _FgLayer, _RaumblockName)) return;
